...
static void G_BuildTouchPreset(touchconfig_t *controls, touchconfigstatus_t *status, touchmovementstyle_e tms, fixed_t scale, boolean tiny)
{
	...
	if (tiny)
	{
		...
		controls[gc_jump].x = (rightcorner - controls[gc_jump].w - corneroffsetxcon - (12 * FRACUNIT));
		controls[gc_jump].y = (bottomcorner - controls[gc_jump].h - corneroffsetycon - (12 * FRACUNIT)) + jsoffs + offs + nonjoyoffs;
	...
	controls[gc_systemmenu].x = (rightcorner - controls[gc_systemmenu].w - corneroffsetxnav);
	controls[gc_systemmenu].y = corneroffsetynav;
	...
	if (status->cantalk)
	{
		...
		controls[gc_talkkey].x = (rightcorner - controls[gc_talkkey].w - corneroffsetxnav);
...
void G_PositionTouchNavigation(void)
{
	...
	INT32 smallwnav = 16 * FRACUNIT;
	INT32 smallhnav = 16 * FRACUNIT;
#endif
	INT32 i;
	...
	{
		nav[KEY_ESCAPE].w = smallwnav;
		nav[KEY_ESCAPE].h = smallhnav;
		nav[KEY_ESCAPE].color = 35;
	}
	else
	{
		nav[KEY_ESCAPE].x = corneroffsetxnav;
		nav[KEY_ESCAPE].y = corneroffsetynav;
		nav[KEY_ESCAPE].w = wnav;
		nav[KEY_ESCAPE].h = hnav;
		nav[KEY_ESCAPE].h = hnav;
	}
	...
	{
		nav[KEY_ENTER].w = wnav;
		nav[KEY_ENTER].h = hnav;
		nav[KEY_ENTER].x = (((vid.width / vid.dupx) * FRACUNIT) - nav[KEY_ENTER].w - corneroffsetxnav);
		nav[KEY_ENTER].y = corneroffsetynav;
	}
	...
	{
		nav[KEY_CONSOLE].x = corneroffsetxnav;
		nav[KEY_CONSOLE].y = nav[KEY_ESCAPE].y + nav[KEY_ESCAPE].h + (8 * FRACUNIT);
		nav[KEY_CONSOLE].w = wnav;
		nav[KEY_CONSOLE].h = hnav;
