...
boolean LUAh_MusicJingle(jingletype_t jingletype, char *newname, UINT16 *musflags, boolean *looping)
{
	hook_p hookp;
	boolean hooked = false;
	...
	for (hookp = roothook; hookp; hookp = hookp->next)
		if (hookp->type == hook_MusicJingle)
		{
			...
			if (lua_isboolean(gL, -3) && lua_toboolean(gL, -3))
				...
				strncpy(newname, lua_tostring(gL, -3), 7);
			// output 2: musflags override
			if (lua_isnumber(gL, -2))
				*musflags = lua_tonumber(gL, -2);
			// output 3: looping override
			if (lua_isnumber(gL, -1))
				*looping = lua_tonumber(gL, -1);
